function myscreen = DFP_ContrastThresholding2_MH(observer,stairStruct,Eye)
...
% but we are stuck on it now.
    ...
    % Staircase 1
    stimulus.stair{1} = upDownStaircase(1,3,stimulus.StartContrast(1),.02, 'levitt'); %1 wrong the contrast goes up, 3 right it goes down, starting at 20% contrast, with an intial step size of 7%
    stimulus.stair{1}.minThreshold = .001;
    stimulus.stair{1}.maxThreshold = 1;
    
    %Staircase 2
    stimulus.stair{2} = upDownStaircase(1,3,stimulus.StartContrast(2),.02, 'levitt');
    stimulus.stair{2}.minThreshold = .001;
    stimulus.stair{2}.maxThreshold = 1;

    stimulus.stair{3} = upDownStaircase(1,3,stimulus.StartContrast(3),.02, 'levitt');
    stimulus.stair{3}.minThreshold = .001;
    stimulus.stair{3}.maxThreshold = 1;

    %Staircase 4
    stimulus.stair{4} = upDownStaircase(1,3,stimulus.StartContrast(4),.02, 'levitt');
