function myscreen = DPF_ExoAttnContrastThresh(observer,ContrastStart,Eye)

%% in this version of the contrast thresholding we have 2 staircases that are spread across all 4 locations. The staircases follow levit rules.
% this means that
global stimulus;
global MGL;
...
stimulus = [];
stimulus.StartContrast=ContrastStart;
...
n_repeats = 25; % 100 trials (25 per location...)

%Just do 1 staircase per location, this is going to be the average
%threhsold of all 4 locations (in a way...). However, we now have 25 trials
%per location, for a total of 100 trials.
[contrast, location, ExoCueCondition, staircase,repeat] = ndgrid(1, 1:4, 2, 1, 1:n_repeats); %1 contrast, 4 locations, 1 neutral cue, 1 staircase, repeat 25 times per location
...
if ~task.thistrial.gotResponse
    ...
    if stimulus.tmp.response
        ...
        stimulus.stair{stimulus.tmp.staircaseNum} = upDownStaircase(stimulus.stair{stimulus.tmp.staircaseNum},1);
...
cellCheck = isa(ContrastStart,'cell'); % checking if lobal variable stairStructure is a cell (containing structures for each staircase)

if stimulus.StartContrast

else
stimulus.StartContrast(1)=.20;
stimulus.StartContrast(2)=.20;
end

% staircases
% Here we define our levitt staircase

stimulus.stair{1} = upDownStaircase(1,3,stimulus.StartContrast(1),.08,'levitt');
stimulus.stair{1}.minThreshold = .01;
stimulus.stair{1}.maxThreshold = 1;
stimulus.stair{1}.minStepsize = 0.01;
stimulus.stair{2} = upDownStaircase(1,3,stimulus.StartContrast(2),.08,'levitt');
stimulus.stair{2}.minThreshold = .01;
stimulus.stair{2}.maxThreshold = 1;
stimulus.stair{2}.minStepsize = 0.01;
