function myscreen = Eyetrack_DPF_ExoAttnExp(observer,IndContrast,Eye)

% revised as of 7.6.17 to incorporate online trial feedback and real-time
% trial replacement
% note: will only run after running 9-pt calibration
...
task{1}.segmin =     [1 Inf .06 .06 .12 .04 .66 5 1 5];
% 1)ITI 2)FIX 3)PRECUE 4)SOA 5)STIM 6)ISI 7)RESP CUE ONLY (cannot respond for 660 ms) 8)RESP WINDOW- cue is still on screen, but participants can now respond; cue is on screen total of 5660 ms (5.66 s) 9) Please fixate online feedback 10)FEEDBACK

task{1}.segmax =     [1 Inf .06 .06 .12 .04 .66 5 1 5];
task{1}.getResponse = [0 0 0 0 0 0 0 1 0];

n_repeats = 6; % 48 trials per block
...
function [task, myscreen] = StartSegmentCallback(task, myscreen);
    ...
    elseif (task.thistrial.thisseg == 2) % fixation
        ...
        stimulus.FixationStarted=0; %%% !!!! See if need to comment out
    ...
    elseif (task.thistrial.thisseg == 9);%%%%% TO ADD FOR ONLINE EYETRACKING
        ...
        %% ExoCueCondition
        if task.randVars.fixBreak(stimulus.trialnum) == 1;
            temp = zeros(1,size(task.randVars.ExoCueCondition,2)+1);
            temp(1:size(task.randVars.ExoCueCondition,2)) = task.randVars.ExoCueCondition;
            temp(end) = task.randVars.ExoCueCondition(stimulus.trialnum);
            task.randVars.ExoCueCondition = temp;
        end
        %% targetLocation
        if task.randVars.fixBreak(stimulus.trialnum) == 1;
...
%     elseif (task.thistrial.thisseg == 10) % response
%         stimulus.trialnum = stimulus.trialnum + 1;
    end
...
%     if stimulus.trialend <= task.numTrials% End of block Feedback
